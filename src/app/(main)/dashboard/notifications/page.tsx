...
import { Bell, CheckCircle2, Filter, FlaskConical, XCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-bold text-2xl text-gray-900 dark:text-gray-100">Notifications</h1>
        ...
        {FILTERS.map(({ value, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              {label}
              <span
                className={`rounded-full px-1.5 py-0.5 text-xs ${active ? "bg-white/20" : "bg-gray-200 dark:bg-gray-700"}`}
              >
                {count}
              </span>
            </button>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        {isLoading ? (
          <p className="px-5 py-8 text-center text-gray-500 text-sm">Loading…</p>
        ) : filtered.length === 0 ? (
              ...
              <li key={n.id} className="flex gap-4 px-5 py-4 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <p className="flex-shrink-0 text-gray-400 text-xs tabular-nums" title={formatDateTime(n.startedAt)}>
                      {formatRelative(n.startedAt)}
                    </p>
                  </div>
                  {n.message && <p className="mt-1 text-gray-600 text-sm dark:text-gray-400">{n.message}</p>}
                  <p className="mt-2 text-gray-400 text-xs tabular-nums">{formatDateTime(n.startedAt)}</p>
                </div>
              </li>
            ))}
          </ul>
      ...
      <p className="text-gray-400 text-xs">
        ...
        <code className="rounded bg-gray-100 px-1 py-0.5 text-xs dark:bg-gray-800">
          [Alert] Kuma service down → Telegram
        </code>
        .
      </p>
    </div>
