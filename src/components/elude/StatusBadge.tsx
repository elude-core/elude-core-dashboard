...
export function StatusBadge({ status, upCount, totalCount }: StatusBadgeProps) {
  ...
  if (status === "loading") {
    ...
    content = (<><Loader2 className="h-4 w-4 animate-spin" /><span>Loading…</span></>);
  } else if (status === "ok") {
    cls = "bg-green-100 font-medium text-green-700 dark:bg-green-900/30 dark:text-green-300";
    content = (<><CheckCircle2 className="h-4 w-4" /><span>All systems operational</span></>);
    ...
    content = (<><AlertTriangle className="h-4 w-4" /><span>{down > 0 ? `${down} service${down > 1 ? "s" : ""} degraded` : "Degraded"}</span></>);
  } else {
    cls = "bg-red-100 font-medium text-red-700 dark:bg-red-900/30 dark:text-red-300";
    content = (<><XCircle className="h-4 w-4" /><span>System down</span></>);
  }
