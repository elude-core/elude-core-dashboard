...
function StatusPill({ status }: { status: KumaMonitor["status"] }) {
  ...
  return (
    <span className={`inline-block rounded-full px-2 py-0.5 font-medium text-xs ${styles[status]}`}>{status}</span>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
      <div className="border-gray-200 border-b px-5 py-4 dark:border-gray-800">
        <h3 className="font-semibold text-base text-gray-900 dark:text-gray-100">Stack Health</h3>
      </div>
      <table className="w-full">
        <thead>
          <tr className="border-gray-100 border-b text-left text-gray-500 text-xs uppercase tracking-wider dark:border-gray-800">
        ...
        <tbody>
          {rows.map(({ monitor, meta }) => (
            <tr key={monitor.id} className="border-gray-100 border-b last:border-0 dark:border-gray-800">
              <td className="px-5 py-3">
                <div className="flex items-center gap-3">
                  <IconByName name={meta?.iconName ?? "HelpCircle"} />
                  <span className="font-medium text-gray-900 text-sm dark:text-gray-100">{monitor.name}</span>
                </div>
              </td>
              <td className="px-5 py-3">
