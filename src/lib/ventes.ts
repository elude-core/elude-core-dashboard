// Module PUR : types, validation de contrat, calcul d'agrégation. Aucun import de
// "./redis" ici — ce fichier est importé tel quel par `VentesClient.tsx` ("use client"),
// et un import de valeur (pas `import type`) tire tout le module dans le bundle
// navigateur. La lecture Redis (`readSnapshot`, réservée au serveur) vit dans
// `./ventes.server` (cf. `src/lib/cookie.client.ts` / `.server-actions.ts` pour la même
// logique de séparation ailleurs dans ce dépôt).
